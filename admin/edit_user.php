...
if(!$username) {
  ...
  $query = $pdo->prepare('SELECT * FROM user WHERE user.username = :username');
  ...
  $nama_lengkap = (isset($result['nama_lengkap'])) ? trim($result['nama_lengkap']) : '';
  $level = (isset($result['level'])) ? trim($result['level']) : '';
...
if(isset($_POST['submit'])):  
  ...
  if(empty($errors)):
    ...
    $prepare_query = 'UPDATE user SET nama_lengkap = :nama_lengkap, level = :level WHERE username = :username';
    $data = array(
    ...
    if($password) {
      $prepare_query = 'UPDATE user SET nama_lengkap = :nama_lengkap, password = :password, level = :level WHERE username = :username';
      $data['password'] = sha1($password);
    }
    $handle = $pdo->prepare($prepare_query);    
    $sukses = $handle->execute($data);
    
    redirect_to('tampil_user.php?status=sukses-edit');
    ...
    <div id="content-wrapper">
      ...
      <div class="container-fluid">
        ...
        <div class="">
                ...
                <center>            
                <h3>Edit User</h3>
                <form action="edit_user.php?username=<?php echo $username; ?>" method="post">
