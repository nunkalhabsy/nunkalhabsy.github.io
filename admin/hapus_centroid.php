...
if(!$id_centroid) {
  ...
  $query = $pdo->prepare('SELECT id_centroid FROM centroid WHERE centroid.id_centroid = :id_centroid');
  ...
  if(empty($result)) {
    ...
    $handle = $pdo->prepare('DELETE FROM centroid WHERE id_centroid = :id_centroid');       
    $asem = $handle->execute(array(
      'id_centroid' => $result['id_centroid']
    ));
    redirect_to('tampil_centroid.php?status=sukses-hapus');
  }
}
?>
