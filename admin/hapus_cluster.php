...
if(!$id_cluster) {
  ...
  $query = $pdo->prepare('SELECT id_cluster FROM cluster WHERE cluster.id_cluster = :id_cluster');
  ...
  if(empty($result)) {
    ...
    $handle = $pdo->prepare('DELETE FROM cluster WHERE id_cluster = :id_cluster');       
    $asem = $handle->execute(array(
      'id_cluster' => $result['id_cluster']
    ));
    redirect_to('tampil_cluster.php?status=sukses-hapus');
  }
}
?>
